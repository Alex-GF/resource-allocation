# Results Discussion: PROMISE vs RAOM4CC vs EdgeWiseCR
...
PROMISE stands alone as the fifth technique category. It uses a constraint programming solver
...
hard constraint types enforced (higher is more complete). PROMISE enforces 12 of 12 hard
constraint types; all baselines enforce 6 of 12.

The Pareto front shifts rightward for PROMISE on VR and Robot (higher solver time), while
remaining near-static for baselines across all four applications. On LiDAR, PROMISE occupies
...
scale, PROMISE's median time (5.65 s) places it far from the frontier, making the constraint
coverage advantage more costly to obtain.

| Constraint | PROMISE | RAOM4CC | EdgeWiseCR |
|:-----------|:-----:|:-------:|:----------:|
...
additional metrics. The six constraints that only PROMISE enforces—provider exclusions,
...
application type. PROMISE enforces 12/12 hard constraints (100 %). RAOM4CC enforces 6/12
...
constraints exclusive to PROMISE are the ones that address provider interoperability, feature
